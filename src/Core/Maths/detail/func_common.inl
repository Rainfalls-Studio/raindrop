...
namespace detail
{
	...
	struct compute_sign
	{
		...
		{
			return vec<L, T, Q>(Raindrop::Core::Math::lessThan(vec<L, T, Q>(0), x)) - vec<L, T, Q>(Raindrop::Core::Math::lessThan(x, vec<L, T, Q>(0)));
		}
	};
...
#	endif
	...
	struct compute_mod
	{
		...
		{
			GLM_STATIC_ASSERT(std::numeric_limits<T>::is_iec559, "'mod' only accept floating-point inputs. Include <Core/Maths/gtc/integer.hpp> for integer inputs.");
	...
	struct compute_step_vector
	{
		...
		{
			return mix(vec<L, T, Q>(1), vec<L, T, Q>(0), Raindrop::Core::Math::lessThan(x, edge));
		}
	};
...
#	endif
	...
	GLM_FUNC_QUALIFIER vec<L, bool, Q> isnan(vec<L, T, Q> const& v)
	{
		...
		for (length_t l = 0; l < v.length(); ++l)
			Result[l] = Raindrop::Core::Math::isnan(v[l]);
		return Result;
	}
...
#	endif
	...
	GLM_FUNC_QUALIFIER vec<L, bool, Q> isinf(vec<L, T, Q> const& v)
	{
		...
		vec<L, bool, Q> Result;
		for (length_t l = 0; l < v.length(); ++l)
			Result[l] = Raindrop::Core::Math::isinf(v[l]);
		return Result;
	}
