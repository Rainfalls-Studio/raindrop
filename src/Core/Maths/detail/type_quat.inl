...
#		endif
	...
	//	vec<3, valType> c = Raindrop::Core::Math::cos(eulerAngle * valType(0.5));
	//	vec<3, valType> s = Raindrop::Core::Math::sin(eulerAngle * valType(0.5));
	//
	...
	{
		vec<3, T, Q> c = Raindrop::Core::Math::cos(eulerAngle * T(0.5));
		vec<3, T, Q> s = Raindrop::Core::Math::sin(eulerAngle * T(0.5));
...
#	endif
	...
	GLM_FUNC_QUALIFIER GLM_CONSTEXPR vec<3, T, Q> operator*(qua<T, Q> const& q, vec<3, T, Q> const& v)
	{
		...
		vec<3, T, Q> const uv(Raindrop::Core::Math::cross(QuatVector, v));
		vec<3, T, Q> const uuv(Raindrop::Core::Math::cross(QuatVector, uv));
	...
	{
		return Raindrop::Core::Math::inverse(q) * v;
	}
