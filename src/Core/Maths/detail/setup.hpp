...
namespace std {
	// Import SYCL's functions into the namespace Raindrop::Core::Math::std to force their usages.
	...
	// the new Raindrop::Core::Math::std namespace.
	template<typename T>
...
#	define GLM_COUNTOF(arr) Raindrop::Core::Math::countof(arr)
...
#	endif
	...
	// Report .length() type
#	if GLM_CONFIG_LENGTH_TYPE == GLM_LENGTH_SIZE_T
#		pragma message("GLM: GLM_FORCE_SIZE_T_LENGTH is defined. .length() returns a Raindrop::Core::Math::length_t, a typedef of std::size_t.")
#	else
#		pragma message("GLM: GLM_FORCE_SIZE_T_LENGTH is undefined. .length() returns a Raindrop::Core::Math::length_t, a typedef of int following GLSL.")
#	endif
