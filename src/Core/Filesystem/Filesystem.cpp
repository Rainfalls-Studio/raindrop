...
namespace Raindrop::Core::Filesystem{
	...
	usize getSize(const Path& path){
		RAINDROP_profile_function();
			...
			HANDLE hFile = CreateFile(path.str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	Memory::Array<String> getFolderContent(Memory::Allocator& allocator, const Path& path){		
		RAINDROP_profile_function();
			...
			StringCchCopy(szDir, MAX_PATH, (STRSAFE_LPCSTR)path.str());
